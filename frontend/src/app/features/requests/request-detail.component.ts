    ...
    <div class="space-y-5 fade-in max-w-5xl">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          @if (incident()) {
            <p class="page-subtitle font-mono">#{{ incident()!.id_incidente }}</p>
          }
        </div>
      </div>
...
export class RequestDetailComponent implements OnInit, OnDestroy {
  ...
  get incidentId(): number {
    return parseInt(this.route.snapshot.paramMap.get('id') || '', 10) || 0;
  }
  ...
  accept(): void {
    ...
    this.incidentsService.assign(this.incidentId, { id_tecnico: this.selectedTecnico ? Number(this.selectedTecnico) : undefined }).subscribe({
      next: () => {
