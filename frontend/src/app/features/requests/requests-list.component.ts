          ...
          @for (inc of filtered(); track inc.id_incidente) {
            <a [routerLink]="['/requests', inc.id_incidente]"
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-xs font-mono" style="color: var(--text-muted);">#{{ inc.id_incidente }}</p>
                @if (inc.resumen_ia) {
...
export class RequestsListComponent implements OnInit {
  ...
  filtered = computed(() => {
    ...
    return this.incidents()
      ...
      .filter(i => !s || i.clasificacion.toLowerCase().includes(s) || String(i.id_incidente).includes(s));
  });
