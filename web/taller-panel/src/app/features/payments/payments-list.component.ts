      ...
      <div class="surface p-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div>
            ...
            <input type="text" [(ngModel)]="busqueda" placeholder="UUID parcial..." class="input text-sm" />
          </div>
        </div>
      </div>
              ...
              <div class="lg:col-span-3 flex items-center gap-2.5">
                ...
                <div>
                  <p class="text-xs font-mono font-semibold" style="color: var(--text-primary);">
                    #{{ p.id_incidente.substring(0,8).toUpperCase() }}
                  </p>
...
export class PaymentsListComponent implements OnInit, OnDestroy {
  ...
  pagosFiltrados = computed(() => {
    ...
    return this.pagos().filter(p =>
      !texto || p.id_incidente.toLowerCase().includes(texto)
    );
  });
