...
export class StatsService {
  ...
  getDashboard(idTenant?: string): Observable<DashboardStats> {
    let params = new HttpParams();
    if (idTenant) params = params.set('id_tenant', idTenant);
    return this.http.get<DashboardStats>(`${this.api}/stats/dashboard`, { params });
  }

  getZonas(idTenant?: string): Observable<ZonaIncidente[]> {
    let params = new HttpParams();
    if (idTenant) params = params.set('id_tenant', idTenant);
