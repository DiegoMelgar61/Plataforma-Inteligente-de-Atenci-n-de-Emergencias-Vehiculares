...
export class AdminService {
  ...
  changeRole(id: string, rol: string): Observable<User> {
    return this.http.patch<User>(`${this.api}/usuarios/${id}/rol`, { rol });
  }

  toggleActive(id: string, activo: boolean): Observable<User> {
  ...
  updateWorkshop(id: string, data: Partial<TallerCreate & { activo: boolean }>): Observable<Taller> {
    return this.http.put<Taller>(`${this.api}/talleres/${id}`, data);
  }

  toggleWorkshop(id: string, activo: boolean): Observable<Taller> {
